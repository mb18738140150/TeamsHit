...
- (void)setupSession
{
    ...
    AVCaptureDeviceInput * vidioInput = [AVCaptureDeviceInput deviceInputWithDevice:device error:nil];
    if (vidioInput == nil) {
        NSLog(@"创建输入设备失败");
        return;
    }
    ...
    if (![session canAddInput:vidioInput]) {
        NSLog(@"无法添加输入设备");
        return;
    }
