...
- (void)refreshAsset{
    ...
    for (NSInteger i = 0; i < self.photos.count; i++) {
        ...
        for (int j = 0; j < self.doneAssets.count; j++) {
            MLSelectPhotoAssets *assetdone = [self.doneAssets objectAtIndex:j];
            if ([assetdone.asset.defaultRepresentation.url isEqual:asset.asset.defaultRepresentation.url ]) {
