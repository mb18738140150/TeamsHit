...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetShouldAntialias(contextRef, YES);// 设置图形上下文的抗锯齿开启或关闭
...
- (void)createCornerWithContextRef:(CGContextRef)contextRef {
    CGContextSetFillColorWithColor(contextRef, UIColorFromRGB(0x12B7F5).CGColor);
    
    CGContextSaveGState(contextRef);
    /*
     CGContextSaveGState 函数作用是将当前图形状态推入堆栈。之后您对图形所做的修改会影响随后的描画操作，但不影响存储在堆栈中的拷贝。，在修改完成后，通过 CGContextRestoreGState 函数，把堆栈顶部的状态弹出，返回到之前的图形状态。这种推入和弹出的方式是回到之前图形状态的快速方法，避免逐个撤销所有的状态修改，这也是将某些状态（比如裁剪路径）恢复到原有设置的唯一方式；
...
//   CGRect clip = CGRectInset(self.clearRect, -margin * 0.4f, -margin * 0.4f);// 该方法也可以
...
- (void)createGridWithContextRef:(CGContextRef)contextRef {
    CGContextSetStrokeColorWithColor(contextRef, UIColorFromRGB(0x12B7F5).CGColor);
